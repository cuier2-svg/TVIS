...
bool BatchPIRClient::cuckoo_hash(vector<uint64_t> batch)
{
    ...
    original_cuckoo_table_ = cuckoo_table_;
    translate_cuckoo();
    return true;
}
...
vector<uint64_t> BatchPIRClient::get_original_cuckoo_table()
{
    return original_cuckoo_table_;
}

void BatchPIRClient::translate_cuckoo()
{
    if (!is_cuckoo_generated_)
    {
        throw std::runtime_error("Error: Cannot translate the data because the cuckoo hash table has not been generated.");
    }

    const auto num_buckets = cuckoo_table_.size();
    const auto default_value = batchpir_params_.get_default_value();

    if (!is_map_set_)
    {
        std::unordered_map<uint64_t, std::vector<size_t>> targets_by_bucket;
        targets_by_bucket.reserve(num_buckets);
        for (size_t bucket = 0; bucket < num_buckets; bucket++)
        {
            if (cuckoo_table_[bucket] != default_value)
            {
                targets_by_bucket[cuckoo_table_[bucket]].push_back(bucket);
            }
        }

        const auto num_candidates = batchpir_params_.get_num_hash_funcs();
        std::vector<uint64_t> positions(num_buckets, 0);
        size_t remaining = targets_by_bucket.size();

        for (uint64_t entry = 0; entry < batchpir_params_.get_num_entries() && remaining > 0; ++entry)
        {
            auto candidates = utils::get_candidate_buckets(entry, num_candidates, num_buckets);
            auto target_it = targets_by_bucket.find(entry);
            for (auto bucket : candidates)
            {
                if (target_it != targets_by_bucket.end() &&
                    std::find(target_it->second.begin(), target_it->second.end(), bucket) != target_it->second.end())
                {
                    cuckoo_table_[bucket] = positions[bucket];
                    --remaining;
                }
                ++positions[bucket];
            }
        ...
        if (remaining != 0)
        {
            throw std::runtime_error("Error: Could not translate all cuckoo table entries.");
        }
        return;
    }
    ...
    for (int i = 0; i < num_buckets; i++)
    {
        ...
        if (cuckoo_table_[i] != default_value)
        {
...
void BatchPIRClient::prepare_pir_clients()
{
    ...
    size_t num_client =
        (num_buckets + per_client_capacity - 1) / per_client_capacity;
