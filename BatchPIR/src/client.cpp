...
std::vector<int> required_galois_steps(const PirParams &pir_params)
{
    ...
    const auto dimensions = pir_params.get_dimensions();
    const auto row_size = pir_params.get_seal_parameters().poly_modulus_degree() / 2;
    const auto gap = row_size / dimensions[0];

    auto add_step = [&](int step) {
        const auto row = static_cast<int>(row_size);
        step %= row;
        if (step > row / 2) {
            step -= row;
        } else if (step < -row / 2) {
    ...
    for (size_t i = 1; i < dimensions[0]; ++i) {
        add_step(-1 * static_cast<int>(i * gap));
    }

    if (dimensions.size() == 3) {
        for (size_t i = 1; i < dimensions[2]; ++i) {
            add_step(-1 * static_cast<int>(i * gap));
        }
    }

    const auto num_slots_per_entry = pir_params.get_num_slots_per_entry();
    const auto num_slots_per_entry_rounded = utils::next_power_of_two(num_slots_per_entry);
    const auto current_fill = gap * num_slots_per_entry_rounded;
    for (size_t k = 1; k < row_size / current_fill; k *= 2) {
        add_step(-1 * static_cast<int>(k * current_fill));
    }

    for (size_t k = 1; k < num_slots_per_entry; ++k) {
        add_step(-1 * static_cast<int>(k * gap));
    }
...
Client::Client(PirParams &pir_params) : pir_params_(pir_params)
{
    ...
    keygen_->create_galois_keys(required_galois_steps(pir_params_), gal_keys_);
...
vector<RawResponses> Client::decode_merged_responses(PIRResponseList response, size_t cuckoo_size, vector<vector<uint64_t>> entry_slot_lists)
{
    ...
    int row_offset = 0;
    const size_t servers_per_response = remaining_fill;
    for (int k = 0; k < response.size(); k++)
    {
        ...
        for (int j = 0; j < servers_per_response; j++)
        {
            const size_t server_idx = (k * servers_per_response) + j;
            if (server_idx >= entry_slot_lists.size())
            {
                break;
            }
            ...
            for (int l = 0; l < entry_slot_lists[server_idx].size(); l++)
            {
                auto tmp = l;
                ...
                int pir_offset = (server_idx * 2 * gap_) + l;
                if (pir_offset >= pir_entries.size())
                {
                    break;
                }

                auto entry_offset = ((entry_slot_lists[server_idx][l] * gap_) + tmp);
