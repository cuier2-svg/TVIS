...
void Server::convert_merge_pir_dbs()
{
    ...
    db_.assign(total_db_plaintexts, std::vector<uint64_t>(polynomial_degree_, 0ULL));

    for (int db_index = 0; db_index < num_databases_; db_index++)
    {
        auto rotate_amount = db_index;
        auto row_offset = 0;
        if (rotate_amount >= gap_)
        {
        ...
        for (int entry_index = 0; entry_index < total_rawdb_entries; ++entry_index)
        {
            auto coeffs = convert_to_list_of_coeff(rawdb_list_[db_index][entry_index]);
            int plaintext_idx = entry_index / pir_dimensions_[0];
            const int slot = (entry_index * gap_) % row_size_;
            const int rotated_slot = row_offset + ((slot + rotate_amount) % row_size_);

            for (int column = 0; column < num_columns_per_entry; column++)
            {
                db_[plaintext_idx][rotated_slot] += coeffs[column];
